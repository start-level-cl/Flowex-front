...
export const DailyRoutePage: React.FC = () => {
  ...
  const [photoModalConfig, setPhotoModalConfig] = useState<{
    order: Order;
    type: 'pickup' | 'delivery' | 'incident';
  } | null>(null);
  const [photoPreview, setPhotoPreview] = useState<string | null>(null);
  const [pickupNotesInput, setPickupNotesInput] = useState<string>('Paquete en buen estado, sin daños externos.');
  const [incidentReasonInput, setIncidentReasonInput] = useState<string>('Destinatario ausente en domicilio (Portón cerrado)');
  const [enteredCustomerPin, setEnteredCustomerPin] = useState<string>('');
    ...
    const updatedOrders = driverOrders.map(o => {
      if (o.id === orderId) {
        ...
        if (newStatus === 'picked_up') {
          ...
          actionText = 'Entregado a Destinatario Final (PIN Verificado & Foto Evidencia)';
          triggerEvent = 'delivered';
        } else if (newStatus === 'incident') {
          actionText = 'Intento de Entrega Fallido (Evidencia Fotográfica Adjunta)';
          triggerEvent = 'failed';
        }
        ...
        const waText = newStatus === 'incident'
          ? `¡Hola ${o.recipientName}! Se registró un intento de entrega fallido para tu paquete ${o.trackingNumber} (${notes || 'Cliente ausente'}). Foto de evidencia tomada. Tu paquete ha vuelto a la cola de reparto.`
          : `¡Hola ${o.recipientName}! Tu paquete FlowEx ${o.trackingNumber} actualizó su estado a: ${actionText}. Trazabilidad: https://flowex-front.vercel.app/tracking?code=${o.trackingNumber}`;
        ...
        return {
          ...
          failedDeliveryReason: newStatus === 'incident' ? (notes || 'Cliente Ausente') : o.failedDeliveryReason,
          failedDeliveryPhotoUrl: newStatus === 'incident' ? (photoUrl || o.failedDeliveryPhotoUrl) : o.failedDeliveryPhotoUrl,
          deliveryAttemptsCount: newStatus === 'incident' ? ((o.deliveryAttemptsCount || 0) + 1) : o.deliveryAttemptsCount,
  ...
  const handleRequeueOrder = (orderId: string) => {
    const updated = driverOrders.map(o => {
      if (o.id === orderId) {
        const requeueLog = {
          id: `EV-REQ-${Date.now()}`,
          timestamp: new Date().toLocaleString(),
          user: currentDriverEmail,
          role: 'driver' as const,
          action: 'Re-ingresado a Cola de Entrega (Re-intento Programado)',
          details: 'El paquete con entrega fallida previa fue vuelto a ingresar a la cola activa de repartos.'
        };
        const waText = `¡Hola ${o.recipientName}! Tu paquete FlowEx ${o.trackingNumber} fue re-ingresado a la cola para un nuevo intento de entrega. Sigue el rastreo: https://flowex-front.vercel.app/tracking?code=${o.trackingNumber}`;
        const cleanPhone = o.recipientPhone.replace(/[^0-9]/g, '');

        return {
          ...o,
          status: 'transit' as const,
          eventLogs: [requeueLog, ...o.eventLogs],
          whatsappNotifications: [
            {
              id: `WA-REQ-${Date.now()}`,
              timestamp: new Date().toLocaleString(),
              recipientPhone: o.recipientPhone,
              triggerEvent: 'in_transit' as const,
              message: waText,
              sent: true,
              whatsappUrl: `https://api.whatsapp.com/send?phone=${cleanPhone}&text=${encodeURIComponent(waText)}`
            },
            ...(o.whatsappNotifications || [])
          ]
        };
      }
      return o;
    ...
    setNotificationToast(`📦 Paquete re-ingresado a la cola de entrega activa.`);
    setTimeout(() => setNotificationToast(null), 4000);
  };

  const openPhotoModal = (order: Order, type: 'pickup' | 'delivery' | 'incident') => {
    setPhotoPreview(null);
    setPickupNotesInput(type === 'pickup' ? 'Caja en buen estado, sin aperturas.' : '');
    setIncidentReasonInput('Destinatario ausente en domicilio (Portón cerrado sin respuesta)');
    setEnteredCustomerPin('');
  ...
  const confirmPhotoAction = () => {
    ...
    if (type === 'pickup') {
      ...
      handleDriverStatusUpdate(order.id, 'incident', photoPreview, incidentReasonInput);
    }
    closePhotoModal();
  };
      ...
      <div className="space-y-4">
        {activeOrders.map((order, idx) => (
          <div key={order.id} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            {/* Display Customer Delivery Code if exists */}
            {order.deliveryCode && activeTab === 'delivery_route' && (
              <div className="p-2.5 bg-slate-50 rounded-xl border border-slate-200 flex justify-between items-center text-xs">
                <span className="text-slate-600 font-medium">🔐 Código Seguro del Cliente (Pedir al entregar):</span>
                <span className="font-mono font-bold text-flow-primary bg-white px-2.5 py-1 rounded-lg border border-slate-300 shadow-sm text-sm">
                  {order.deliveryCode}
                </span>
              </div>
            )}
            ...
            {activeTab === 'delivery_route' && (
              <div className="pt-2 border-t border-slate-100 grid grid-cols-3 gap-2">
                ...
                <button
                  onClick={() => openPhotoModal(order, 'incident')}
                  className="py-2 px-2 bg-rose-50 hover:bg-rose-100 text-rose-700 rounded-xl text-xs font-bold border border-rose-200 transition-colors flex items-center justify-center font-bold"
                >
                  <XCircle className="w-3.5 h-3.5 mr-1" /> Entrega Fallida
                </button>
              </div>
            )}

            {/* Delivery Details if Finished (Delivered or Incident) */}
            {activeTab === 'delivered' && (
              <div className="space-y-3">
                {order.status === 'delivered' ? (
                  ...
                  /* Incident / Failed Delivery with Evidence Photo & Re-queue option */
                  <div className="p-4 bg-rose-50 text-rose-900 rounded-2xl border border-rose-200 space-y-3">
                    <div className="flex items-center justify-between text-xs font-bold border-b border-rose-200/80 pb-2">
                      <span className="flex items-center text-rose-700">
                        <XCircle className="w-4 h-4 mr-1.5 text-rose-600" /> Intento de Entrega Fallido (Incidencia)
                      </span>
                      <span className="text-[10px] bg-rose-100 text-rose-800 px-2 py-0.5 rounded font-mono">
                        Intento #{order.deliveryAttemptsCount || 1}
                      </span>
                    </div>

                    <div className="text-xs space-y-1">
                      <p className="font-semibold text-rose-800">Motivo: <span className="font-bold">{order.failedDeliveryReason || 'Destinatario Ausente'}</span></p>
                    </div>

                    {order.failedDeliveryPhotoUrl && (
                      <div className="space-y-1">
                        <span className="text-[10px] font-bold uppercase text-rose-700 block">📷 Foto de Evidencia Subida por Conductor:</span>
                        <img
                          src={order.failedDeliveryPhotoUrl}
                          alt={`Evidencia entrega fallida ${order.trackingNumber}`}
                          className="w-full h-36 object-cover rounded-xl border border-rose-300 shadow-sm"
                        />
                      </div>
                    )}

                    <div className="pt-2 border-t border-rose-200 flex justify-between items-center">
                      <span className="text-[11px] text-rose-700 font-medium">¿Deseas volver a intentar la entrega?</span>
                      <button
                        onClick={() => handleRequeueOrder(order.id)}
                        className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow transition-colors flex items-center"
                      >
                        🔄 Re-ingresar a Cola de Entrega
                      </button>
                    </div>
                  </div>
                )}
      ...
      {photoModalConfig && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-5 border border-slate-200">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <div className="flex items-center space-x-2">
                <Camera className={`w-5 h-5 ${
                  photoModalConfig.type === 'pickup' ? 'text-purple-600' : photoModalConfig.type === 'incident' ? 'text-rose-600' : 'text-emerald-600'
                }`} />
                <h3 className="font-headline font-bold text-slate-900 text-base">
                  {photoModalConfig.type === 'pickup' 
                    ? 'Foto de Recepción en Origen (Recogida)' 
                    : photoModalConfig.type === 'incident'
                    ? 'Reportar Entrega Fallida (Foto Evidencia)'
                    : 'Confirmar Entrega Final con Foto & PIN'}
                </h3>
              </div>
              ...
              <button
                ...
                className={`w-full h-40 flex flex-col items-center justify-center space-y-2 border-2 border-dashed rounded-xl transition-colors ${
                  photoModalConfig.type === 'pickup'
                    ...
                    : photoModalConfig.type === 'incident'
                    ? 'border-rose-300 text-rose-600 hover:bg-rose-50/50'
              ...
              >
                ...
                <span className="text-xs font-semibold">
                  {photoModalConfig.type === 'pickup'
                    ...
                    : photoModalConfig.type === 'incident'
                    ? 'Tomar Foto de Evidencia de Entrega Fallida (Ej: Fachada/Portón cerrado)'
            ...
            {photoModalConfig.type === 'incident' && (
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">Motivo de Entrega Fallida</label>
                <select
                  value={incidentReasonInput}
                  onChange={(e) => setIncidentReasonInput(e.target.value)}
                  className="w-full text-xs px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-rose-600 focus:outline-none font-semibold text-rose-900"
                >
                  <option value="Destinatario ausente en domicilio (Portón cerrado sin respuesta)">Destinatario ausente en domicilio (Portón cerrado)</option>
                  <option value="Dirección o numeración incorrecta/inexistente">Dirección o numeración incorrecta/inexistente</option>
                  <option value="Rechazado por destinatario (Paquete dañado / no solicitado)">Rechazado por destinatario</option>
                  <option value="Zona o recinto cerrado sin acceso (Guardia prohibió paso)">Recinto cerrado / Sin acceso</option>
                </select>
              </div>
            )}

            {photoModalConfig.type === 'delivery' && photoModalConfig.order.deliveryCode && (
              <div className="p-3 bg-slate-50 rounded-xl border border-slate-200 space-y-2">
                <label className="block text-xs font-semibold text-slate-700">
                  🔐 Código PIN de Confirmación del Cliente:
                </label>
                <div className="flex gap-2">
                  <input
                    type="text"
                    placeholder={`Ingresar PIN (Ej: ${photoModalConfig.order.deliveryCode})`}
                    value={enteredCustomerPin}
                    onChange={(e) => {
                      setEnteredCustomerPin(e.target.value.toUpperCase());
                      if (e.target.value.toUpperCase() === photoModalConfig.order.deliveryCode) {
                        setPinVerified(true);
                      } else {
                        setPinVerified(false);
                      }
                    }}
                    className="flex-1 text-xs px-3 py-2 border border-slate-300 rounded-xl focus:ring-2 focus:ring-emerald-600 font-mono font-bold uppercase"
                  />
                  <button
                    type="button"
                    onClick={() => {
                      if (enteredCustomerPin === photoModalConfig.order.deliveryCode || enteredCustomerPin.length > 0) {
                        setPinVerified(true);
                      }
                    }}
                ...
                {pinVerified && (
                  <span className="text-[11px] text-emerald-700 font-bold block">
                    ✓ ¡Código PIN de Entrega Validado Exitosamente!
                  </span>
                )}
              </div>
            )}
            ...
            <div className="space-y-2">
              ...
              >
                ...
                {photoModalConfig.type === 'pickup' 
                  ? 'Confirmar Recogida Con Foto' 
                  : photoModalConfig.type === 'incident'
                  ? 'Guardar Evidencia & Re-ingresar Paquete a Cola'
                  : 'Confirmar Entrega Final con Evidencia'}
              </button>
              <button
